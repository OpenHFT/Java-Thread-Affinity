...
=== Core Affinity Control (net.openhft.affinity.Affinity)
...
=== CPU Lock Management (net.openhft.affinity.AffinityLock)
...
=== CPU Layout Detection (net.openhft.affinity.CpuLayout)
...
=== High-Resolution Timer (net.openhft.ticker.Ticker)
...
=== OS-Specific Implementations (net.openhft.affinity.impl)
...
** *macOS*: Process ID, Thread ID via JNA (`OSXJNAAffinity`).
No affinity modification; `getCpu()` returns -1.
** *Solaris*: Process ID, Thread ID via JNA (`SolarisJNAAffinity`).
No affinity modification; `getCpu()` returns -1.
...
=== Affinity Thread Factory (net.openhft.affinity.AffinityThreadFactory)
...
=== Native Code Compilation (C/C{pp})

* *FR12.1*: The system _shall_ include C/C{pp} source code for native functions required for affinity and timer operations on Linux and macOS.
...
* *FR12.2*: A Makefile _shall_ be provided to compile the native C/C{pp} code into a shared library (`libCEInternals.so`).
...
* *NFR2. Dependencies*:
** *JNA*: `net.java.dev.jna:jna`, `net.java.dev.jna:jna-platform`.
Version 5.x or higher is recommended for full functionality.
The project currently uses version 4.4.0 (as per README, though POMs might show updates).
** *SLF4J API*: `org.slf4j:slf4j-api` for logging.
** *JetBrains Annotations*: `org.jetbrains:annotations` for code quality.
* *NFR3. Performance*: The library _should_ introduce minimal overhead.
Native calls _should_ be efficient.
The primary goal is to enable performance improvements in the client application.
...
** Native components _shall_ be implemented in C/C{pp}.
* *NFR7. Usability*:
...
* *Native Code (`src/main/c`)*: C/C{pp} sources for `libCEInternals.so` providing functions like `getAffinity0`, `setAffinity0` (Linux JNI), `rdtsc0`.
...
== Native Components (libCEInternals.so)
...
** *macOS*: (Separate C file `software_chronicle_enterprise_internals_impl_NativeAffinity_MacOSX.c`) Uses `pthread_mach_thread_np`, `thread_policy_get`, `thread_policy_set`.
Note: JNA implementations are generally preferred on macOS.
